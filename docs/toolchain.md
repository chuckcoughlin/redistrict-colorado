...
Use the directory containing the un-modularized jar as the starting point. The  modularized result will be stored into ``lib``.
```
...
The core of the application is based on the open source project [OpenJUMP](http://www.openjump.org).
Code has been extensively modified and simplified for our purposes. Over 12,000 source files were pared down to about 400. Modifications include:
...
The main code for analyzing ESRI shape files is derived from the Unified Mapping Platform, [OpenJump](https://sourceforge.net/projects/jump-pilot/files/latest/download). Code at version 1.14.1 was simplified and upgraded for Java 13.
