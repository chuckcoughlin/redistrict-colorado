package org.openjump.coordsys.srid;


import java.io.UnsupportedEncodingException;

import org.openjump.coordsys.Unit;
...
public class SRSInfo {
    ...
    private String source = ""; // The source of SRS information (ex. prj file
                                // path)
    private Registry registry = SRSInfo.Registry.EPSG; // The registry in which this SRS is
                                      // referenced
    ...
    private Registry guessRegistry(String code) {
        ...
        Registry registry = SRSInfo.Registry.SRID; // Used if code is negative or non-numeric
        if (code != null && code.matches("\\d+")) {
            int srid = Integer.parseInt(code);
            if (srid < 32768 || srid > 5999999) {
                registry = SRSInfo.Registry.EPSG;
            } else if (srid > 32999 && srid < 200000) {
                registry = SRSInfo.Registry.ESRI;
            }
        }
        return registry;
    }
