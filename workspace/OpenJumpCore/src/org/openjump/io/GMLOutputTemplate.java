    ...
    public void load(java.io.Reader r, String readerName)
        ...
        while (keepgoing && ((line = getLine(buffRead)) != null)) {
            if ((index = line.indexOf("<%")) != -1) {
                ...
                if (index2 == -1) {
                    throw new ParseException(String.format("While trying to find the GML output header %s line %d number %d",
                ...
                if (!(token.equalsIgnoreCase("FEATURE"))) {
                    throw new ParseException(String.format("While trying to find the GML output header found token that isnt a feature  %s line %d number %d",
                        streamName, lineNumber, index));
                }
        ...
        if (line == null) {
            throw new ParseException(String.format("Unexpected EOF while looking for header %s line %d number %d",
                streamName, lineNumber, index));
        }
        ...
        while (keepgoing) {
            if ((index = line.indexOf("<%")) != -1) {
                ...
                if (index2 == -1) {
                    throw new ParseException(String.format("While looking at the GML feature text,  %s line %d number %d",
                        streamName, lineNumber, index));
                }
                ...
                if (token.equalsIgnoreCase("ENDFEATURE")) {
                    ...
                    if (!(validop(token))) {
                        throw new ParseException(String.format("invalid token %s: %s line %d number %d",
            ...
            if (keepgoing) {
                ...
                if (line == null) {
                    throw new ParseException(String.format("Unexpected EOF while looking for feature %s line %d number %d",
                        streamName, lineNumber, index));
                }
            }
        }
