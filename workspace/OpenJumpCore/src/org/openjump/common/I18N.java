...
public final class I18N {
  private static final String CLSS = "I18N";
  ...
  public static Locale fromCode(final String localeCode) {
    ...
    if (lc.length > 1) {
      ...
      LOGGER.fine("lang:" + lc[0] + " " + "country:" + lc[1]);
      locale = new Locale(lc[0], lc[1]);
    } else if (lc.length > 0) {
      LOGGER.fine("lang:" + lc[0]);
      locale = new Locale(lc[0]);
    } else {
      LOGGER.fine(localeCode
...
//        System.out.println(label);
    try {
      ...
      LOGGER.severe(e.getMessage() + " no default value, the resource key is used: "
  ...
  private String getValue(final String key) {
      ...
      try {
        ...
        if (!missing.contains(key)) {
          ...
          LOGGER.severe(msg + e.getLocalizedMessage());
