...
public class FeatureSchema implements Cloneable, Serializable {
   ...
   */
    ...
    public int getAttributeIndex(String attributeName) {
        ...
        if (index == null) {
            throw new IllegalArgumentException(String.format("%s: Attribute (%s) unrecognized",CLSS,attributeName));
        }
        return index;
    }
    ...
    public void addAttribute(String attributeName, AttributeType attributeType) {
        if (AttributeType.GEOMETRY == attributeType) {
            ...
            if (geometryIndex != -1) {
            	throw new IllegalArgumentException(String.format("%s: only one geometry is authorized",CLSS));
            }
            geometryIndex = attributeCount;
        }
        //Assert.isTrue(!attributeNames.contains(attributeName));
        if (attributeNames.contains(attributeName)) {
        	throw new IllegalArgumentException(String.format("%s: Attribute (%s) already exists",CLSS,attributeName));
        }
