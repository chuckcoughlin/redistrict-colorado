...
public class CRSHelper {
	private static final String CLSS = "CRSHelper";
            ...
            if (pm == null) {
                try {
                    ...
                    try {
                        ...
                        LOGGER.severe(pmName + " prime meridian is not parsable");
                        return null;
                    }
                }
            }
        ...
        if (nadgrids != null) {
            ...
            for (String grid : grids) {
                if (!grid.equalsIgnoreCase("null")) {
                    LOGGER.warning("A grid has been found.");
                    if (grid.equalsIgnoreCase("@null")) {
                        ...
                        try {
                            ...
                            LOGGER.severe(String.format("%s.setNadgrids: Cannot find the nadgrid %s (%s)",CLSS,grid ,ex.getLocalizedMessage()));
                        }
                    }
                }
            }
    ...
    private static Ellipsoid getEllipsoid(Map<String, String> param) {
        ...
        if (ellps == null) {
            LOGGER.warning("Ellipsoid cannot be defined");
        }
        return ellps;
    }
    ...
    private static Projection getProjection(String projectionName, Ellipsoid ell,
        ...
        if (sk != null && sk_0 != null) {
            if (!sk.equals(sk_0)) {
                LOGGER.warning("Two different scales factor at origin are defined, the one chosen for the projection is k_0");
            }
        }
