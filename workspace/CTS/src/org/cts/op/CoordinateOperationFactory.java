...
public final class CoordinateOperationFactory {
	private static final String CLSS = "CoordinateOperationFactory";
    ...
    public static Set<CoordinateOperation> createCoordinateOperations(
        ...
        if (sourceDatum == null) {
            LOGGER.warning(source.getName() + " has no Geodetic Datum");
        ...
        if (targetDatum == null) {
        	LOGGER.warning(target.getName() + " has no Geodetic Datum");
        ...
        if (opList.isEmpty()) {
        	LOGGER.warning("No transformation found from " + source.getCode() + " to " + target.getCode());
    ...
    private static void addCoordinateOperations(
        ...
        try {
        	...
        	LOGGER.warning("Operation from " + source.getCode() + " to " + target.getCode() + " could not be created");
        	LOGGER.severe("CoordinateOperationFactory "+ e.getLocalizedMessage());
        }
    }
            ...
            if (!datumTransformations.isEmpty()) {
                for (CoordinateOperation datumTransformation : datumTransformations) {
                    try {
                        ...
                        LOGGER.warning("Operation from " + source.getCode() + " to " + target.getCode()
            	...
            	LOGGER.warning("Cannot create a CoordinateOperation from :\n" + source + "\nto :\n" + target);
            }
        }
