...
import org.openjump.workbench.component.ValidatingTextField;
...
public class WorkbenchUtilities {
  ...
  // AbstractDriver, they should be moved to AbstractDriver. WorkbenchUtilitiesities is
  ...
  public static boolean showConfirmOverwriteDialog(Component parent, File file) {
    int response = JOptionPane
        .showConfirmDialog(parent, I18N.getMessage(
            "ui.WorkbenchUtilities.overwrite-prompting", file.getName()),
    ...
    return new FileFilter() {
      ...
      public boolean accept(File f) {
        ...
        for (String ext : extensions) {
          if (WorkbenchUtilities.getExtension(f).equalsIgnoreCase(ext)) {
            return true;
          }
        }
  ...
  public static Cursor createCursorFromIcon(Image iconImage) {
    // Don't use WorkbenchUtilities#resize, which uses SCALE_SMOOTH, which
  ...
  public static Cursor createCursor(Image image, Point hotSpot) {
    ...
    return Toolkit.getDefaultToolkit().createCustomCursor(image, hotSpot,
        I18N.get("ui.WorkbenchUtilities.jump-workbench-custom-cursor"));
  }
