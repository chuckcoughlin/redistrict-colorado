...
import com.vividsolutions.jump.workbench.Action.Configuration;
...
public class ExtensionsAboutPanel extends JPanel {
  ...
  public ExtensionsAboutPanel(ActionManager mgr) {
    this.ActionManager = mgr;
    try {
      jbInit();
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  public void refresh() {
    ...
    // user extensions
    for (Iterator i = ActionManager.getConfigurations().iterator(); i.hasNext();) {
      Configuration configuration = (Configuration) i.next();
      String msg = ActionManager.message(configuration);
      sb.append("<div><b>"
          + GUIUtil.escapeHTML(ActionManager.name(configuration), false, false)
          + "</b> "
          + GUIUtil.escapeHTML(ActionManager.version(configuration), false,
              false)
