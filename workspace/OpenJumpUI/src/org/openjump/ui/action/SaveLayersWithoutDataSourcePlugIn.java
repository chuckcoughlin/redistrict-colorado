...
import org.openjump.ui.component.JFCWithEnterAction;
...
import org.openjump.workbench.component.MultiInputDialog;
import org.openjump.workbench.model.Layer;
import org.openjump.workbench.model.Task;

import com.vividsolutions.jump.io.datasource.DataSource;
import com.vividsolutions.jump.io.datasource.StandardReaderWriterFileDataSource;
import com.vividsolutions.jump.workbench.Action.MultiEnableCheck;
import com.vividsolutions.jump.workbench.model.LayerView;

/**
* Subclass this to implement a 'Save Project' Action.
*/
public class SaveLayersWithoutDataSourceAction extends AbstractAction {
    
    private static final String KEY = SaveLayersWithoutDataSourceAction.class.getName();
    
    private static final String LAYERS_WITHOUT_DATASOURCE = I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.layers-without-datasource-management");

    private static final String DONOTSAVE = I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.do-not-save");
    private static final String SAVEASJML = I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.save-as-jml");
    private static final String SAVEASSHP = I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.save-as-shp");

    private static final String FILECHOOSER = I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.directory-chooser");

    private static final String WARN_USER = I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.every-layer-has-a-datasource");
    
    public static final ImageIcon ICON = IconLoader.icon("disks_dots.png");
    private JFileChooser fileChooser;
    
    public SaveLayersWithoutDataSourceAction() {
    }
    ...
    public boolean execute(ActionContext context, Collection<Layer> collection, File dir) throws Exception {
        ...
        dialog.addSubTitle(I18N.getMessage("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.layers-without-datasource", 
            collection.size()))
            .setToolTipText(tooltip);
        dialog.addLabel(I18N.get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.hover-the-label-to-see-the-list"))
            .setToolTipText(tooltip);
        ...
        WorkbenchUtilities.centreOnWindow(dialog);
        ...
        if (dialog.wasOKPressed()) {
            if (dialog.getBoolean(DONOTSAVE)) {
                ...
                for (Layer layer : collection) {
                    ...
                    if (dataSource != null) {
                        if (file.exists()) {
                            if (WorkbenchUtilities.showConfirmOverwriteDialog(context.getWorkbenchFrame(), file)) {
      ...
      enableCheck.add(new EnableCheck() {
        public String check(javax.swing.JComponent component) {
          return layersWithoutDataSource(wc.getTask()).size() > 0 ? null
              : I18N
                  .get("org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction.a-layer-without-datasource-must-exist");
        }
      });
      return enableCheck;
    }
