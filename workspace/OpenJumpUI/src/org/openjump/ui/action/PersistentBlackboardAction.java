...
public class PersistentBlackboardAction extends AbstractAction {
    private static String persistenceDirectory = ".";

    private static String fileName = "workbench-state.xml";

    private static final String BLACKBOARD_KEY = PersistentBlackboardAction.class
            .getName()
            + " - BLACKBOARD";
    ...
    public void initialize(final ActionContext context) throws Exception {
      ...
      try {
        get(workbenchContext).putAll(
            ((Blackboard) new XML2Java(workbenchContext.getWorkbench()
                .getActionManager().getClassLoader()).read(bufferedReader,
