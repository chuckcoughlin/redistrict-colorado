...
import org.openjump.workbench.model.UndoableCommand;
import org.openjump.workbench.task.TaskMonitorManager;
import org.openjump.workbench.ui.EditTransaction;
 ...
 */
public abstract class AbstractAction implements Action, ShortcutEnabled, EnableChecked, Iconified, Recordable {
	private static final String CLSS = "AbstractAction";
  ...
  protected void execute(UndoableCommand command, ActionContext context) {
   ...
   */
  protected void reportNothingToUndoYet(ActionContext context) {
  ...
  protected boolean isRollingBackInvalidEdits(ActionContext context) {
    return PersistentBlackboardAction.get(context.getWorkbenchContext())
  ...
  public Icon getIcon(Dimension dim) {
    ...
    if (icon instanceof ImageIcon && dim != null
      ...
      icon = WorkbenchUtilities.resize((ImageIcon) icon, dim.height);
    }
  ...
  public static String createName(Class plugInClass) {
    try {
      ...
      LOGGER.severe(e.getMessage() + " " + plugInClass.getName());
  ...
  public static ActionListener toActionListener(final Action plugIn,
    ...
    return new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        try {
          ...
          ActionContext ActionContext = workbenchContext.createActionContext();
          ...
          try {
            boolean executeComplete = plugIn.execute(ActionContext);
            ...
            if (plugIn instanceof ThreadedAction && executeComplete) {
              taskMonitorManager
                  .execute((ThreadedAction) plugIn, ActionContext);
            }
          } finally {
            if (undoableEditReceiver != null) {
   ...
   * @return enable check
   */
  public static EnableCheck getEnableCheck(Action plugin) {
   ...
   */
  public static Action[] fetchShortcutEnabledPlugins(Action plugin) {
    ...
    if (plugin instanceof MultiShortcutEnabled) {
      Action[] shortys = ((MultiShortcutEnabled) plugin)
          .getShortcutEnabledPlugins();
      if (shortys != null)
        plugins.addAll(Arrays.asList(shortys));
    }

    return (Action[]) plugins.toArray(new Action[]{});
  }
   ...
   * @return done
   */
  public static boolean registerShortcuts(Action plugin) {
    Action[] shortys = fetchShortcutEnabledPlugins(plugin);
    if (shortys.length < 1)
      return false;

    JUMPWorkbench wb = JUMPWorkbench.getInstance();
    for (Action p : shortys) {
