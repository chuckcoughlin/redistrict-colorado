...
public class OptionsAction extends AbstractAction {
  ...
  private static OptionsAction instance = null;

  public boolean execute(ActionContext context) throws Exception {
    reportNothingToUndoYet(context);
    WorkbenchUtilities.centreOnWindow(dialog(context));
  ...
  private static OptionsDialog dialog(ActionContext context) {
  ...
  public void initialize(ActionContext context) throws Exception {
    ...
    dialog(context).addTab(
        I18N.get("ui.Action.OptionsAction.view-edit"),
        WorkbenchUtilities.resize(IconLoader.icon("edit.gif"), 16),
        new EditOptionsPanel(PersistentBlackboardAction.get(context.getWorkbenchContext())));
    dialog(context).addTab(
        I18N.get("ui.Action.OptionsAction.snap-vertices-tools"),
        WorkbenchUtilities.resize(IconLoader.icon("QuickSnap.gif"), 16),
    ...
    dialog(context).addTab(I18N.get("ui.Action.OptionsAction.selection-style"),
    		WorkbenchUtilities.resize(IconLoader.icon("Select.gif"), 16),
  ...
  public static boolean execute() throws Exception {
    ActionContext pc = JUMPWorkbench.getInstance().getContext().createActionContext();

    if (instance == null) {
      OptionsAction p = new OptionsAction();
      try {
        p.initialize(pc);
        instance = p;
      } catch (Exception e) {
