...
public class SaveProjectAsAction extends AbstractSaveProjectAction {
    ...
    public static final String FILE_CHOOSER_DIRECTORY_KEY = 
        SaveProjectAsAction.class.getName() + " - FILE CHOOSER DIRECTORY";
    
    public static final FileFilter JUMP_PROJECT_FILE_FILTER =
        WorkbenchUtilities.createFileFilter(I18N.get("ui.Action.SaveProjectAsAction.jump-project-files"),
                                 new String[]{"jmp", "jcs"});
    ...
    public void initialize(ActionContext context) throws Exception {
        ...
        fileChooser = WorkbenchUtilities.createJFileChooserWithOverwritePrompting("jmp");
        fileChooser.setDialogTitle(I18N.get("ui.Action.SaveProjectAsAction.save-project"));
        WorkbenchUtilities.removeChoosableFileFilters(fileChooser);
        fileChooser.addChoosableFileFilter(JUMP_PROJECT_FILE_FILTER);
        fileChooser.addChoosableFileFilter(WorkbenchUtilities.ALL_FILES_FILTER);
        fileChooser.setFileFilter(JUMP_PROJECT_FILE_FILTER);
        Blackboard blackboard = PersistentBlackboardAction.get(context.getWorkbenchContext());
    ...
    public String getName() {
        return I18N.get("ui.Action.SaveProjectAsAction.save-project-as");
    }
    ...
    public boolean execute(ActionContext context) throws Exception {
        ...
        if (collection.size() > 0) {
            // Starting with OpenJUMP 1.4.1beta (2011-04-20), the Action uses
            // org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction
            // to give the user the possibility to save unsaved layers to HD
            // before saving the project
            new org.openjump.core.ui.Action.file.SaveLayersWithoutDataSourceAction()
        ...
        PersistentBlackboardAction.get(context.getWorkbenchContext())
