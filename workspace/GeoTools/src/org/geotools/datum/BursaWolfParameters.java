...
import org.geotools.operation.matrix.Matrix4;
...
public class BursaWolfParameters implements Cloneable, Serializable {
	private static final String CLSS = "BursaWolfParameters";
    ...
    public void setAffineTransform(final Matrix matrix, final double eps)
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (Math.abs(difference) > eps) {
                throw new IllegalArgumentException(String.format("%s.setAffineTransform: Not an affine transform", CLSS));
            }
        }
    ...
    public boolean equals(final Object object) {
        if (object instanceof BursaWolfParameters) {
            ...
            return     this.dx == that.dx
                    ...
                    && this.ez == that.ez
                    && this.ppm == that.ppm
                    && this.targetDatum == that.targetDatum;
        }
        return false;
    }
}
